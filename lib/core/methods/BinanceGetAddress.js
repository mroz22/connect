...
var BinanceGetAddress = /*#__PURE__*/function (_AbstractMethod) {
  (0, _inheritsLoose2["default"])(BinanceGetAddress, _AbstractMethod);

  function BinanceGetAddress(message) {
    var _this;
    ...
    _this.firmwareRange = (0, _paramsValidator.getFirmwareRange)(_this.name, (0, _CoinInfo.getMiscNetwork)('BNB'), _this.firmwareRange); // create a bundle with only one batch if bundle doesn't exists
    ...
    if (bundle.length === 1) {
      _this.info = "Export Binance address for account #" + ((0, _pathUtils.fromHardened)(_this.params[0].path[2]) + 1);
    } else {
      _this.info = 'Export multiple Binance addresses';
    }

    return _this;
  }

  var _proto = BinanceGetAddress.prototype;
        ...
        while (1) {
          switch (_context3.prev = _context3.next) {
            ...
            case 2:
              ...
              _context3.next = 7;
              return this.device.getCommands().binanceGetAddress(batch.path, false);
            ...
            case 14:
              _context3.next = 16;
              return this.device.getCommands().binanceGetAddress(batch.path, batch.showOnTrezor);
