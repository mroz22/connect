...
var getFirmwareRange = function getFirmwareRange(method, coinInfo, currentRange) {
  ...
  if (coinInfo) {
    if (!coinInfo.support || typeof coinInfo.support.trezor1 !== 'string') {
      current['1'].min = '0';
    } else if ((0, _deviceFeaturesUtils.versionCompare)(coinInfo.support.trezor1, current['1'].min) > 0) {
      current['1'].min = coinInfo.support.trezor1;
    }

    if (!coinInfo.support || typeof coinInfo.support.trezor2 !== 'string') {
      current['2'].min = '0';
    } else if ((0, _deviceFeaturesUtils.versionCompare)(coinInfo.support.trezor2, current['2'].min) > 0) {
  ...
  if (range) {
    ...
    if (min) {
      if (current['1'].min === '0' || (0, _deviceFeaturesUtils.versionCompare)(current['1'].min, min[0]) < 0) {
        current['1'].min = min[0];
      }

      if (current['2'].min === '0' || (0, _deviceFeaturesUtils.versionCompare)(current['2'].min, min[1]) < 0) {
        current['2'].min = min[1];
      }
    }

    if (max) {
      if (current['1'].max === '0' || (0, _deviceFeaturesUtils.versionCompare)(current['1'].max, max[0]) < 0) {
        current['1'].max = max[0];
      }

      if (current['2'].max === '0' || (0, _deviceFeaturesUtils.versionCompare)(current['2'].max, max[1]) < 0) {
        current['2'].max = max[1];
      }
    }
  }
